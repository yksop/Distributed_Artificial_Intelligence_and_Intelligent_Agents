...
species Guest skills: [moving, fipa] {
	...
	string itemToBuy <- one_of(sellable_items);
	...
	reflex participate_in_auction when: willing_to_auction and !empty(cfps) {
		...
		string typeOfAuction <- content_list[1];
		if (my_max_price >= offered_price and typeOfAuction = itemToBuy) {
			write "I am willing to buy " + itemToBuy + " and I am agent " + name + "!";
			do propose with: [message: cfp, contents: [offered_price]];
			willing_to_auction <- false;
		} else {
			do refuse with: [message: cfp, contents: ['Price too high or not interested in buying this item']];
		}
...
species Auctioneer skills: [fipa] {
	...
	string itemToBeSold <- one_of(sellable_items);
	...
	reflex start_auction when: !auction_running and !item_sold and time_counter >= wait_before_start {
		...
		do start_conversation to: list(Guest where (each.itemToBuy = itemToBeSold)) protocol: "fipa-contract-net" performative: "cfp" contents: [current_price, itemToBeSold];
